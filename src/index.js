...
const CONFIG = {
  ...
  maxOpenTrades: 1,
  logInterval: parseInt(process.env.LOG_INTERVAL) || 300000 // 5 min
};

// ============ ÉTAT DU BOT ============
const state = {
  candles: [],
  currentPosition: null, // { id: 'firestore_id', type, entryPrice, contractId, timestamp }
  isRunning: true,
  lastSignal: null,
  isProcessing: false,
  dailyStats: {
...
function connectDeriv() {
  ...
  connection.onopen = async () => {
    ...
    try {
      ...
      // Enregistrer le résumé journalier à minuit
...
async function runStrategy() {
  ...
  try {
    ...
    await saveIndicators({
      ...
      ma15: ma15.toFixed(4),
      ma100: ma100.toFixed(4),
      ma200: ma200.toFixed(4),
      atr: atr.toFixed(4),
      price: currentClose.toFixed(4),
      distanceFromMA200: distanceFromMA200.toFixed(4),
    ...
    if (isValidBuy && state.lastSignal !== 'BUY' && !state.currentPosition) {
      console.log(`📈 SIGNAL BUY | Prix: ${currentClose.toFixed(2)} | MA200: ${ma200.toFixed(2)}`);
      await executeTrade('BUY', currentClose);
      state.lastSignal = 'BUY';
    } 
    else if (isValidSell && state.lastSignal !== 'SELL' && !state.currentPosition) {
      console.log(`📉 SIGNAL SELL | Prix: ${currentClose.toFixed(2)} | MA200: ${ma200.toFixed(2)}`);
...
async function closeTrade(contractId, positionId, entryPrice, exitPrice) {
  try {
    ...
    // ===== ENREGISTRER LE TRADE DANS FIREBASE =====
    await saveTrade({
      symbol: CONFIG.symbol,
